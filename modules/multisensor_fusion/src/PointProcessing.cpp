...
// ===================================================
//      读取 lidar - camera  标定参数，及 camera 内参
// ===================================================

CalibrationData PointProcessing::loadCalibrationFromYAML(const std::string& yamlFile,
                                        const std::string& lidar_type)    // "A4" or "B2"
{
    int car_id;
    YAML::Node cfg = YAML::LoadFile(yamlFile);
    car_id = cfg["car_id"].as<int>();

    std::string car_key = "car" + std::to_string(car_id);
    if (!cfg["cars"][car_key]) {
        throw std::runtime_error("Car ID not found in YAML!");
    }

    const YAML::Node& node = cfg["cars"][car_key][lidar_type];

    CalibrationData calib;

    // ----------- RT 4x4 -----------
    calib.RT = cv::Mat::zeros(4, 4, CV_32F);
    auto RT_list = node["RT"].as<std::vector<float>>();
    for (int i = 0; i < 16; i++) {
        calib.RT.at<float>(i / 4, i % 4) = RT_list[i];
    }

    // ----------- intrinsic 3x4 -----------
    calib.intrinsic = cv::Mat::zeros(3, 3, CV_32F);
    auto intr_list = node["intrinsic"].as<std::vector<float>>();
    for (int i = 0; i < 9; i++) {
        calib.intrinsic.at<float>(i / 3, i % 3) = intr_list[i];
    }

    // ----------- distortion Nx1 -----------  
    auto dist_list = node["distortion"].as<std::vector<float>>();
    int dlen = dist_list.size();
    calib.distortion = cv::Mat::zeros(1, dlen, CV_32F);
    for (int i = 0; i < dlen; i++) {
        calib.distortion.at<float>(0, i) = dist_list[i];
    }
...
float PointProcessing::pointToRoadDistance(
    const PointXYZ &p,
    const pcl::PointCloud<pcl::PointXYZ>::Ptr &road_point_list)
{
    if (!road_point_list || road_point_list->empty()) {
        return std::numeric_limits<float>::max();
    }

    int N = road_point_list->points.size();
    if (N == 1) {
        return std::hypot(p.x - road_point_list->points[0].x,
                          p.y - road_point_list->points[0].y);
    }

    // Step1. 先根据轨迹起点粗估索引
    const auto &A0 = road_point_list->points[0];
    float dx = p.x - A0.x;
    float dy = p.y - A0.y;
    
    float approx_dist = std::sqrt(dx*dx + dy*dy);
    int idx_est = static_cast<int>(approx_dist / 0.5f);

    if (idx_est < 0) idx_est = 0;
    if (idx_est >= N) idx_est = N - 1;

    // Step2. 只检查 idx_est ±5 范围内的点
    const int K = 5;
    int start_i = std::max(0, idx_est - K);
    int end_i   = std::min(N - 1, idx_est + K);

    float minDist = std::numeric_limits<float>::max();

    // Step3. 遍历局部轨迹点，直接算点与点距离
    for (int i = start_i; i <= end_i; ++i) {
        const auto &R = road_point_list->points[i];
        float dx2 = p.x - R.x;
        float dy2 = p.y - R.y;
