...
void InitSocket() {
    ...
    // vlan
	const char *interface_name = "eth0.14"; 
    ...
    opt.imr_ifindex = if_nametoindex("eth0.14");// B1虚拟网卡
...
void LidarReceiveMsopThread() {
    ...
    int16_t FovVerticalAngleOffset[4][96] = {0};
    int recvLen;
    ...
    while (true) {
        ...
        if (recvLen > 0) //
        {
			...
			for (int block = 0; block < 2; block++)  // 1 packet:12 data blocks
			{
				...
				for (int dsr = 0, k = 0; dsr < SCANS_PER_BLOCK; dsr++, k += 5)  // 96  3
				{
					...
					if (last_azimuth - azimuth_corrected_f > 90 && point_num > 3000)
					{
