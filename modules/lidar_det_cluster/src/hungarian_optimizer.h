...
void HungarianOptimizer<T>::Minimize(std::vector<std::pair<size_t, size_t>>* assignments) {
  /*
  算法初始化:OptimizationInit();
            其主要完成了代价矩阵“膨胀补 0”、代价矩阵标记初始化、增广路初始化等工作。
  */  
  OptimizationInit();

  DoMunkres();//运行算法主流程 DoMunkres
  FindAssignments(assignments);//生成匹配结果 
  OptimizationClear();//清除初始化标志
}
...
void HungarianOptimizer<T>::OptimizationInit() {
  ...
  //代价矩阵标记初始化
  marks_.Resize(matrix_size_, matrix_size_);//用于查看矩阵中的某个元素是否是0*、0‘或者是NONE
  for (size_t row = 0; row < matrix_size_; ++row) 
  {
        for (size_t col = 0; col < matrix_size_; ++col) 
        {
             marks_(row, col) = Mark::NONE;  //Mark { NONE, PRIME, STAR };
        }
  }


  stars_in_col_.assign(matrix_size_, 0);//用于查看当前列是否有0*
  rows_covered_.assign(matrix_size_, false);//用于查看当前行是否被覆盖了
  cols_covered_.assign(matrix_size_, false);//用于查看当前列是否被覆盖了
  assignments_.resize(matrix_size_ * 2);//assignments_是存放0*、0‘索引号的容器
...
/* 
   在列中查找是否含有0*，如果有，则返回0*所在的行，如果没有返回-2
 */
template <typename T>
...
{
  int max_num_iter = 1000;//最大迭代次数
  int num_iter = 0;
  // fn_state_ 代表算法当前状态，即执行到了哪一步   fn_state_：私有成员变量初始化为 -> 空指针
  fn_state_ = std::bind(&HungarianOptimizer::ReduceRows, this);
  /*
  从步骤 1 开始循环迭代，直至求解成功或达到指定迭代次数阈值
    如果fn_state_=nullptr则表示迭代结束，其实fn_state_=nullptr可以视为结束迭代的标志位；
  */
  while ( (fn_state_ != nullptr) && (num_iter < max_num_iter) ) 
  {
    ...
    /*
       fn_state_()主要执行以下步骤：
       1、ReduceRows()：代价矩阵的每一行元素减去该行元素中的最小值；
       2、StarZeroes()：对代价矩阵中所在行和列中都没有标星 0 的 0 元素标记星号 *；
       3、
    */
    ++num_iter;
/*    std::cout<<"num_iter"<<num_iter<<std::endl;*/
  }
  /* 若达到指定迭代次数阈值，则取消最终的代价矩阵中存在不止一个标星 0 的行里的所有星号
   * 标记，实现部分匹配输出 */  
...
void HungarianOptimizer<T>::CheckStar() 
{
  ...
  for (size_t row = 0; row < height_; ++row)//遍历行
  {
        //判断一行里面是否有两个0*
        int  star_col = -1;
        bool is_single = true;
        int  counter = 0;
        for (size_t col = 0; col < width_; ++col) //遍历列
        {
/*              if (IsStarred(row, col)) */
/*              {*/
/*                    if (star_col == -1) */
...
/* Step 1:
 代价矩阵的每一行元素减去该行元素中的最小值，然后执行步骤2
 */
template <typename T>
void HungarianOptimizer<T>::ReduceRows() 
{
   
    for (size_t row = 0; row < matrix_size_; ++row)//遍历每一行
    {
        //初始化最小值
...
/* 
    Step 2:
    对代价矩阵中所在"行"和"列"中都没有标星 0 的 0 元素 标记星号 *，然后执行步骤3；
    若某行有0*意味着，该行被覆盖；
    若某列有0*意味着，该列被覆盖；
*/
template <typename T>
...
{
  for (size_t row = 0; row < matrix_size_; ++row)//遍历行
  {
        if (RowCovered(row)) 
        {
            continue;
        }
        for (size_t col = 0; col < matrix_size_; ++col)//遍历列
        {
              if (ColCovered(col)) 
              {
                    continue;
...
void HungarianOptimizer<T>::CoverStarredZeroes() 
{
    ...
    for (size_t col = 0; col < matrix_size_; ++col)//遍历每一列
    {
        /*
          bool ColContainsStar(const size_t col) const 
          {
               return stars_in_col_[col] > 0;//每列中星号数量
          }
        */
        //一般的，一个列中要么有且只有一个0*，要么没有0*
        if (ColContainsStar(col)) //ColContainsStar(col)函数的作用是：查看当前列是否有0*
        {
          CoverCol(col);
          num_covered++;
        }
    ...
    if (num_covered >= matrix_size_) //情况1：若所有的列都已被覆盖则结束迭代
    {
        fn_state_ = nullptr;
        return;
    }
...
void HungarianOptimizer<T>::PrimeZeroes() 
{
      for (;;) //一个循环只查看一个矩阵元素
      {
            ...
            size_t zero_col = 0;
//在步骤3中覆盖了含有0*的列后            
            if (!FindZero(&zero_row, &zero_col))//情况1 ：如果矩阵未被覆盖的区域没有0元素
            {
                fn_state_ = std::bind(&HungarianOptimizer::AugmentPath, this);
                return;
            }
            else// 情况2：如果矩阵未被覆盖的区域：有0元素
            {
                Prime(zero_row, zero_col);//对为标记的0元素标记为0'
                //判断0'所在行是否有0*
                int star_col = FindStarInRow(zero_row);//如果有则返回列索引号，如果没有返回-2
                
                if (star_col != kHungarianOptimizerColNotFound)//如果0'所在行有0*，把该0*的列覆盖变为行覆盖
                {
                  CoverRow(zero_row);
                  UncoverCol(star_col);
                } 
                else //如果0'所在行没有0*，进行增广操作
                {
                  //zero_row，zero_col为0'的索引号
...
//************************************************寻找增广路(寻找0',最后的那个0'升级为0*)********************
  while (!done) 
  {
        //查看0'所在列是否有0*
        int row = FindStarInCol(assignments_[count].second);//有则返回0*所在行，没有则返回负值
        //🤓️
        if (row != kHungarianOptimizerRowNotFound)//若0'所在的列有0*
        {
        ...
        //🤓️
        if (!done)//若0'所在的列有0*，则把0*所在行的0'找出
        {
              int col = FindPrimeInRow(assignments_[count].first);//查看0*所在行的有0'，FindPrimeInRow函数返回列的索引号
              count++;
              //获取0'的索引号
  ...
  for (size_t i = 0; i <= count; ++i) 
  {
        ...
        if (IsStarred(row, col))//取消增广路中标星 0 的标记
        {
             Unstar(row, col);
        } 
        else //将增广路中的标撇 0 重新标记为标星 0；
        {
            Star(row, col);
        }
  }

  //最后，取消代价矩阵中的所有行列覆盖，清除代价矩阵中所有标撇 0 的标记，返回步骤 3。
  ClearCovers();
  ClearPrimes();
  fn_state_ = std::bind(&HungarianOptimizer::CoverStarredZeroes, this);
}
...
void HungarianOptimizer<T>::AugmentPath() 
{
  ...
  T minval = FindSmallestUncovered();//查询未被覆盖区域的最小值

  /*
    1:未覆盖的矩阵元素 减去minval
    2:被覆盖两次(即被行覆盖又被列覆盖)的矩阵元素 加上minval
    3:被覆盖一次的矩阵元素，不参与运算
    以上两个需求，需要两个for循环才能一起实现
  */
  for (size_t row = 0; row < matrix_size_; ++row) //实现了被覆盖的行 加上minval
  {
    if (RowCovered(row)) 
    {
  ...
  for (size_t col = 0; col < matrix_size_; ++col) //实现了未被覆盖的列 减去minval
  {
    if (!ColCovered(col)) 
    {
